...
export async function POST(request: Request) {
  ...
  const answer = one<{ id: string; userId: string }>("SELECT * FROM Answer WHERE id = ?", body.answerId);
  ...
  run("INSERT INTO ThreadMember (id, threadId, userId) VALUES (?, ?, ?)", id(), threadId, answer.userId);
  run("INSERT INTO ThreadMember (id, threadId, userId) VALUES (?, ?, ?)", id(), threadId, admin.id);
  run(
  ...
  run("UPDATE Answer SET status = 'REPLIED' WHERE id = ?", answer.id);
